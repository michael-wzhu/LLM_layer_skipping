...
class TransformerHparams(object):
    ...
    def get_embedding_flops(self, output=False):
        """Get the forward-pass FLOPs the transformer inputs or output softmax (LM head)."""
        embedding_flops = {}
        if output:
...
if __name__ == "__main__":
    gpt2_calc = TransformerHparams(1280, 36, s=512, v=50257, output_frac=1.0)
    print(gpt2_calc.get_block_flops())
    print(gpt2_calc.get_embedding_flops(output=True))
    print(gpt2_calc.get_embedding_flops(output=False))
